...
def register(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        messages.success(
            request,
            'Your library card is ready. Welcome to The Reading Room!'
        )
...
def home(request):
    if request.session.pop('library_entry_pending', False):
        ...
        request.GET.get('entrance') == '1'
        or not (
            request.user.is_authenticated
            or request.session.get('library_guest')
        )
    ):
    ...
    borrowed_ids = set(
        loans.values_list('book_id', flat=True)
    )
    ...
    return render(
        ...
        'accounts/home.html',
        {
            ...
            'has_sample_books': (
                all_books
                .filter(isbn__startswith='SAMPLE')
                .exists()
            ),
            'query': query,
            'category': category,
